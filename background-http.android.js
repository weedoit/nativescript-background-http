...
function setRequestOptions(request, options) {
    ...
    if (displayNotificationProgress) {
        ...
        if (options.androidAutoClearNotification) {
            uploadNotificationConfig.getCompleted().autoClear = true;
        }

        if (options.androidNotificationMessages) {
            // completed notification configuration
            if (options.androidNotificationMessages.completed) {
                uploadNotificationConfig.getCompleted().message = options.androidNotificationMessages.completed;
            }

            // progress notification configuration
            if (options.androidNotificationMessages.progress) {
                uploadNotificationConfig.getProgress().message = options.androidNotificationMessages.progress;
            }

            // error notification configuration
            if (options.androidNotificationMessages.error) {
                uploadNotificationConfig.getError().message = options.androidNotificationMessages.error;
            }

            // cancelled notification configuration
            if (options.androidNotificationMessages.cancelled) {
                uploadNotificationConfig.getCancelled().message = options.androidNotificationMessages.cancelled;
            }
        }
